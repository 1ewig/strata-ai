...
> Canonical ground-truth document for AI agents and engineers. Written from current source state (Next.js 16.2.10 / React 19.2.7); verify claims against the referenced files before editing. Supersedes `docs/SUMMARY.md`, which predates several hardening passes (server-side history pruning, provider-metadata sanitization, tool-input delta coalescing, DB-backed quota).
...
├── docs/                      — Legacy SUMMARY.md + AI SDK tutorial guide (superseded by root file).
└── src/
