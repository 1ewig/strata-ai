...
- **Autonomous Agentic Workspace Tools**: 8 schema-validated tools enabling the AI to inspect, read, create, surgically edit, rename, and delete workspace documents, plus perform real-time Tavily web searches (`webSearch` — basic depth by default for fast, credit-efficient lookups, with domain/time filters and raw-content mode) and deep Markdown page extraction (`extractUrl`).
...
- **Context Compaction (`/compact`)**: A slash command that streams a dense, structured summary of the conversation + workspace state into a new `metadata.isCompactedSummary` message via `POST /api/agent/compact` (auth-gated, consumes 1 quota message). Both agent endpoints prune history server-side after the latest summary, so later turns never re-read pre-summary context, and the active context-window meter resets to reflect the trimmed history.
