...
- **Word-Paced Smooth Streaming, Live Markdown & Token Fade**: Powered by `smoothStream` (25ms pacing), server-side `coalesceToolInputDeltas()` transform (buffers tool argument chunks to prevent AI SDK partial JSON re-parsing freezes), and `SmoothStreamText` with 60ms throttled Markdown AST batching, an AST remark plugin (`createTokenFadePlugin`) with CSS token opacity & blur fade transitions (`animate-token-fade`, 750ms), and an active streaming caret to ensure continuous, formatted, and natural token flow without jarring chunk bursts.
