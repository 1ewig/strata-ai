...
- **Agent stream assembly:** `lib/ai/agent-runner.ts` (`runAgentResponse`) owns all `streamText` configuration — model resolution, system-prompt re-injection, tool wiring, streaming transforms (`smoothStream` + `coalesceToolInputDeltas` to prevent AI SDK partial JSON re-parsing freezes on tool args), `stopWhen` step cap, lifecycle logging, and SSE wrapping + quota headers. `/api/agent` is a thin HTTP/auth/validation shell that only delegates to it.
