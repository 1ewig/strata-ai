...
- **Operational / non-functional posture:**
  ...
  - **Performance:** Word-paced streaming (`smoothStream`, 25ms) + live Markdown AST streaming via `SmoothStreamText` with throttled AST batching (60ms) and token opacity & blur fade (`animate-token-fade`, 750ms), metadata-only system prompt to minimize token context, observer-driven auto-scroll, `React.memo` on hot chat components.
