...
- **Operational / non-functional posture:**
  ...
  - **Performance:** Word-paced streaming (`smoothStream`, 25ms) + `SmoothStreamText` token opacity fade (`animate-token-fade`, 450ms), metadata-only system prompt to minimize token context, observer-driven auto-scroll, `React.memo` on hot chat components.
...
2. `smoothStream({ delayInMs: 25, chunking: "word" })` paces token delivery so the UI reads as continuous prose, not bursty chunks. `ChatBubble` delegates active text segment rendering to `SmoothStreamText`, which diffs incoming stream updates and animates newly arrived token deltas with a 450ms opacity & blur fade (`animate-token-fade`). The `prepareStep` hook re-injects `buildSystemInstruction(mutableFiles)` before each agent step so the model's file-state view is always current.
