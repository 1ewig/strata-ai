...
- **Prompt** (`buildCompactionInstruction` in `lib/ai/prompts.ts`): instructs Gemini 3.1 Flash Lite (High Effort) to act as the "Master Context Compaction & Memory Synthesis Engine", retaining lossless technical context (requirements, negative constraints, architectural patterns, exact symbols/functions/files), capturing workspace inventory & tool intelligence, and emitting an exhaustive self-contained GFM summary across 6 strict sections (`### 1. Executive Summary & Core Mission`, `### 2. User Requirements, Rules & Constraints`, `### 3. Architecture, Technical Decisions & Key Patterns`, `### 4. Workspace State & File Inventory`, `### 5. Chronological Progress & Key Solutions`, `### 6. Active State, Pending Work & Next Steps`). Like the agent prompt, it receives **metadata-only** file listings.
